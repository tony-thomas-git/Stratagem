...
6. **Resolve the board adapter, then launch the autonomy loop.** First, **board-adapter resolution (neutral, presence-checked):** if a board adapter is configured for this run — e.g. a board plugin is installed and enabled exposing a board-sync skill — set `boardSync` to that adapter's skill name; otherwise `boardSync = null`. The launcher names no external system; it threads a handle it was given (the same delegation as `ledgerPath`). Then, **notifier-adapter resolution (neutral, presence-checked — the outbound twin of board-adapter resolution):** if a notifier adapter is configured for this run — e.g. a notifier/chat plugin is installed and enabled exposing a notify-sync skill — set `notifier` to that adapter's skill name; otherwise `notifier = null`. As with `boardSync`, the launcher names no external system (no chat vendor or channel); it only threads a handle it was given. Then invoke the **Workflow** tool — `name: 'autonomy-loop'` (fallback `scriptPath: ~/.claude/workflows/autonomy-loop.js`) — with `args: { planPath, taskNumbers, budget, branch, integrationVerify, ledgerPath, boardSync, notifier }`. Invoking `/if` IS the user's explicit opt-in to run this multi-agent Workflow. The loop runs each task as discrete real-skill stages (`/px` -> `/ax` -> verifier -> `/ex`->`/fx` on fail) under the budget guard, then a plan-level integration gate (`## Integration-Verify:`) before declaring the feature complete.
...
- **Plan-level completion gate (Phase 4)** — after every per-task gate passes, the loop runs the plan's `## Integration-Verify:` once in a fresh instance (the local stand-in for "external CI/board green"): exit 0 → feature complete, non-zero → terminal `integration` halt. Absent header → **skipped with a surfaced warning**, never silently. `/if`-only — interactive `/phx` is human-verified at close.
